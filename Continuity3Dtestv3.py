...
#Chain of 3D segments, possibly flipped

df = pd.read_pickle('/home/shared_data/helicalc_params/conductivity_test/line_segments_3d_v3.pkl')
...
def check_3d_flip(vector1, vector2, segment1_output, segment2_input, verbose):
        x = check_3d_continuity(segment1_output, segment2_input, verbose = True)
        if x :
...
for i in range(0,length):
    if i ==  0:
        ...
        distancevector = [df['output_x'].iloc[i] - df['input_x'].iloc[i],df['output_y'].iloc[i] - df['input_y'].iloc[i], df['output_z'].iloc[i] - df['input_z'].iloc[i]]
        distancevector = np.array(distancevector)
        x = check_3d_continuity(segment1_output, segment2_input, verbose=False)
        y = check_3d_flip(vector1, distancevector, segment1_output, segment2_input, verbose = False)
...
data = {'connected_to_previous': continuity, 'flipped_to_previous': flipped}
testv3df = pd.DataFrame(data)

#print(df['connected_to_previous'] ==  testv2df['connected_to_previous'])
print((df['connected_to_previous'] ==  testv3df['connected_to_previous']).sum(), len(df))
print((df['flipped_to_previous'] ==  testv3df['flipped_to_previous']).sum(), len(df))
print(df[df['flipped_to_previous'] != testv3df['flipped_to_previous']])